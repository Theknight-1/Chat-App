...
const DiscoverCard: React.FC<CardProps> = ({ title, description, imageUrl, buttonText = "Learn More" }) => {
    return (
        <div className="dark:bg-[#232428] shadow-lg rounded-2xl cursor-pointer w-full md:max-w-xs transition-transform transform">
            <div className="relative w-full h-40">
                <Image src={imageUrl} alt={title} layout="fill" objectFit="cover" className="rounded-t-lg" loading="lazy" />
                <div className="absolute left-5 -bottom-5 p-1 dark:bg-[#232428] rounded-lg">
                    <Image src={imageUrl} alt={title} height={100} width={100} objectFit="cover" className="w-12 h-12 rounded-lg" loading="lazy" />
                </div>
            </div>
            <div className="p-5">
